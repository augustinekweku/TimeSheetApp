    ...
    <div class="staff">
        ...
        <div class="timer">
            <div class="timer_body">
                ...
                <h1 style="padding:0 5px ;" v-html="setTime"></h1>
            </div>
        </div>
        <div class="searchDateRange">
            <div>
                Between:
                <input v-model="fromDate" type="date" style="width:95px" /> and
                : <input v-model="toDate" type="date" style="width:120px" />
            </div>
        </div>
        ...
        <div v-if="showRecordsRange" class="text-center date_range_results">
            <p class="text-center">
                Showing Attendance records from <span>{{ fromDate }}</span> to
                <span>{{ toDate }}</span>
            </p>
        </div>
        <div class="print_buttons">
            <div class="sort_by_verdict">
                <Select
                    @on-change="selectVerdict"
                    v-model="selectedVerdict"
                    placeholder="Sort by verdict"
                    style="width:170px;"
                    class="py-2"
                >
                    <Option
                        v-for="item in verdictList"
                        :value="item.value"
                        :key="item.value"
                        >{{ item.label }}</Option
                    >
...
export default {
    ...
    methods: {
        async selectVerdict() {
            axios
                .get(`app/getSortedVerdicts/${this.selectedVerdict}`)
                .then(result => {
        ...
        async searchDateRange() {
            ...
            axios
                .get(`app/get_date_range/${this.fromDate}/${this.toDate}`)
                .then(result => {
        ...
        async setTimer() {
            if (this.reportingTime.trim() == "")
                return this.error("Time is not set");

            axios
                .post(`app/create_reportingTime/${this.reportingTime}`)
                .then(result => {
                    if ((result.status = 201)) {
                        this.setTime = this.reportingTime;
                    }
        ...
        async reportlist() {
            ...
            const get_reportedTimes = await this.callApi(
                "get",
                "app/get_reportedTimes"
            );
    ...
    async created() {
        const get_reportingTime = await this.callApi(
            "get",
            "app/get_reportingTime"
        );
