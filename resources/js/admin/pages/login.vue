<template>
    <div :class="darkMode ? 'darkMode' : ''">
        <div class="container">
            ...
            <div class="userTypeButton">
                <Dropdown>
                    ...
                    <DropdownMenu slot="list">
                        <a @click="showAdminLogin"
                            ><DropdownItem>Admin</DropdownItem></a
                        >
                        <a @click="showStaffLogin"
            ...
            <div
                v-show="admin_login"
                class="login _1adminOverveiw_table_recent _box_shadow _border_radious _mar_b30 _p20 col-md-4"
            >
            ...
            >
                <div class="login_header">
                    ...
                    <p>
                        Reporting Time:
                        <span class="text-info">{{ reportingTime }}</span>
                    </p>
                    <div class="space"></div>
                </div>
                <div class="space">
            ...
            <Input type="password"  v-model="data.password"  placeholder="Password" />                        
                ...
                <div class="login_footer">
                    <Button
                        ...
                        :loading="isClockin"
                        @click="clockin"
                        >{{ isClockin ? "Clocking in" : "Sign in" }}</Button
                    >
                </div>
            </div>
        </div>
    ...
    methods: {
        ...
        async login() {
            ...
            if (this.data.userType.trim() == "")
                return this.error("Choose User Type");
        ...
        async clockin() {
            if (this.data.staff_id.trim() == "")
                return this.error("Staff ID is required");
            this.isClockin = true;
            const res = await axios.post(
                `app/create_reportedTime/${this.data.staff_id}`
            );
            this.isClockin = true;
            if (res.status === 201) {
                ...
                if (res.data.status == 1) {
                    this.i(
                        `You were ${humanizeDuration(res.data.timediff)} early`,
                        "Clocked in!"
                    );
                } else {
                    this.i(
                        `You were ${humanizeDuration(res.data.timediff)} late`,
                        "Clocked in!"
                    );
                }
            } else {
